...
(function($) {
	$.fn.styleSwitcher = function(options){		
		var defaults = {	
			slidein: false, preview: false, container: this.selector, directory: "css/", useCookie: false, cookieExpires: 30, manageCookieLoad:true
		};
		...
		if (opts.useCookie && opts.manageCookieLoad) {
			...
			if(isCookie){
				...
				$("link[id=gppstyles]").attr("href",newStyle);
				baseStyle = newStyle;
			}
			else{
		...
		var baseStyle = $("link[id=gppstyles]").attr("href");
		if(opts.preview){
			$(opts.container + " a").hover(
				...
				function () {
					$("link[id=gppstyles]").attr("href",baseStyle);
				}
			);
		}
